...
Redes Neurais são sistemas computacionais que visam replicam o modelo de processamento do cérebro. Há diversas variações de Redes Neurais, nesta seção será abordada a mais simples, Redes Neurais FeedForward, serão apresentadas Redes Neurais Convolucionais na subseção \ref{sec:convolucionais}.

Redes Neurais FeedForward são compostas de camadas de neurônios sucessivamente interligadas por sinapses. As sinapses funcionam como um ponderador linear dos neurônios da camada anterior. Os neurônios, por sua vez, representam uma função de ativação, normalmente não linear, tais como: tangente hiperbólica, \textit{sigmoid}, etc. Na figura \ref{fig:neural-net} apresenta-se o diagrama da uma rede, esta conta com uma camada de entrada que representa os dados do problema, seguindo tem-se duas camadas intermediarias, comumente chamadas de escondidas, finalizando na camada de saída que terá o resultado final de todo processamento da rede.
